...
- **Ontology pathfinding**: Parses `dotString` into adjacency map via regex (matches both `->` directed and `--` undirected edges; normalizes multi-label DOT names like `"FEMALE:\ndance (20%)"` to keys `"FEMALE|dance"`). For undirected graphs (`graph G { ... -- ... }`), adds edges in both directions and generates `-[]-` edges in GQL (no arrows). Builds reverse adjacency for bidirectional BFS. Chains through waypoints finding sub-paths. For directed graphs, respects edge direction (`dir: 'fwd'` → `->`, `dir: 'rev'` → `<-`). Edge label constraints use OR matching against the user's full selection. `labelsToKeys` sorts results to prefer exact matches over partial (e.g., `actor` before `actor|director`) to avoid incorrect edge selection
...
- **Label selection uses combo keys**: Selecting a multi-label row like `["director","actor"]` stores a single combo key `"actor|director"` (sorted, pipe-joined). CanvasGraph filters match a node's full sorted label set against these combo keys — prevents `["director","actor"]` selection from also matching single-label `["actor"]` nodes
- **Color maps use combo keys with n:/e: prefixes**: `labelColorMap` stores `"n:actor"`, `"n:actor|director"`, `"e:acted"` etc. Node and edge charts assign PALETTE indices independently (both start from idx 0). Multi-label nodes get their exact combo's color. `getNodeLabelColor`/`getEdgeLabelColor` parse raw JSON labels to combo keys for lookup
- **Graph directed flag** (`graphDirected`) extracted from `/show/graph` response `directed` field during `loadGraphDetails` — no extra API call. Passed to QueryPanel for undirected GQL generation even without ontology loaded
