...
- `MapView` — Canvas-based renderer for WKT geospatial graphs (auto-selected when `identifierType === 'wkt'`). Renders edges as lines from WKT POINT data. Performance-optimized for large graphs (~1M edges): pre-parsed WKT→Float64Array coords, color-batched drawing (one beginPath/stroke per color), rAF-throttled rendering, adaptive LOD decimation (targets ~5K edges at coarsest level during low-zoom interaction, disabled at zoom>=4x where viewport culling suffices), cached canvas dimensions. Zoom (mouse wheel, cursor-centered), pan (drag), HiDPI rendering. Data bounding box (dashed orange rectangle) shown during interaction. Edge picking queries original edge source table via `fetchEdgeDetail` (by edge ID or WKT match). Coordinates shown on edge pick. Label filtering, viewport culling. Info overlay shows visible/total edge count + LOD level. `⤢` maximize / `▣ Restore` button
