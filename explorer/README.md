...
- **Floating query panels** — Multiple independent panels with minimize (`–` → Q1/Q2 pill in header), maximize, restore, and close. State preserved on minimize/restore. **Title-bar drag is clamped to the viewport** — at least 32 px of bar stays visible vertically and 60 px horizontally, so the panel can never be dragged off-screen and lost. Restored positions are also snapped back into reach on mount.
