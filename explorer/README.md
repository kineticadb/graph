...
- **Geo graph renderers** — Renderer toggle (`Auto | Canvas | Deck.gl | WMS`) available for WKT graphs:
  ...
  - **WMS** — Kinetica `/wms` server-side tile rendering. No entity fetch needed. Style sub-toggle: `Auto | Heat | Raster | CB`. Heatmap uses the jet colormap with a zoom-aware blur radius so the color spectrum stays consistent as you zoom in. CB shows class-break coloring by edge label (top 20 labels). Zoom (cursor-centered wheel) + pan. Label filtering via `LABEL_FILTER`. **Edge picking**: click on the tile to run a server-side `STXY_DWITHIN` query around the cursor — picked record lands in the detail strip with the full source-table row. **Auth-aware tile fetch**: deployments behind Basic auth return 401 on plain `<img src>` requests (browsers can't attach an `Authorization` header to image GETs); the tile loader uses `fetch()` with the header and turns the response into a blob URL the `<img>` displays. **Round-trip data preservation**: switching from Deck.gl/Canvas → WMS → back doesn't refetch — the entity-mode `graphTableData` is stashed on WMS entry and restored on exit.
...
- **Click anywhere on the Deck.gl map** (edge or empty area) → copies just `<lon> <lat>` (space-separated, no WKT wrapper) to the clipboard. Open a Constants-mode WKT field in the Solve / Match Helper — the focus-fill drops in the `ST_GEOMFROMTEXT('POINT(x y)')` template with `x y` pre-selected, so Ctrl+V replaces exactly those coords. Overlay shows `✓ copied`. Edge clicks still surface the edge's source-table row in the detail strip.
...
- **Solution table** field is auto-prefilled with `<schema>.solution_<graph>_<solver>` (e.g. `ki_home.solution_san_francisco_shortest_path`) and refreshes on solver-type change unless you've customized the name. **Two SQL forms** keyed on the field:
  - **Value set** → `EXECUTE FUNCTION SOLVE_GRAPH(... SOLUTION_TABLE => '<t>' ...)` (persists results; auto-drop prefix on consecutive runs).
  - **Field cleared** → `SELECT * FROM SOLVE_GRAPH(...)` (and `SELECT * FROM MATCH_GRAPH(...)` for Match) — returns rows directly without materializing a persistent table.
...
- **OPTIONS** section auto-populates with the grammar defaults for the selected solver — every option whose `filtervalues` explicitly mentions the current solver type is pre-filled with its default value (e.g., `match_isochrone` pre-loads `max_radius = 0.0` per the grammar). Switching solver replaces these grammar-supplied rows with the new solver's defaults; rows you add or edit yourself survive untouched. The key datalist is filtered to options applicable to the current solver (e.g., `convergence_limit` only under `PAGE_RANK`).
- **Boolean options** (per grammar `type: 'boolean'`) render as a `true`/`false` dropdown so common toggles like `inverse_solve` / `output_clusters` / `force_undirected` flip with a click.
...
- **Focus-fill templates in Constants mode** — clicking into an empty constants-mode field auto-drops the alias-appropriate template (e.g., `ST_GEOMFROMTEXT('POINT(-122 37)')` for WKT, `'Bill'` for NAME, `1, 2, 3, …` for ID) and pre-selects the editable bit (the coords / body / quoted value) so a single keystroke replaces it.
- **Output dropdown** next to the Solution table input — when the current solver/method has multiple output schemas (e.g. `match_isochrone` has two: x/y/z and POLYGON), a small `Output: [default | 1: … | 2: …]` picker appears with each schema's columns summarized (and `(wkt)` flagged on geometry columns). Picking a non-default value injects `result_table_index = 'N'` into the generated OPTIONS KV_PAIRS. No need to add it manually in the OPTIONS section.
...
- **Video visualization**: when the solver/matcher response includes an inline animated SVG (in `info.svg`), a **Video** tab appears next to Map with the SVG rendered directly — its own SMIL / CSS animations play in place. Common for supply-demand routes, GPS map-matching trails, and similar timeline outputs.
- **Map visualization** (Deck.gl): a **Map** tab appears whenever the rows include either a WKT column (`wktroute` / `wkt` / `polygon` / `geom`, or any LINESTRING/POLYGON/MULTI*/POINT WKT) or a plain x,y coordinate pair (`x`+`y`, `lon`+`lat`, `longitude`+`latitude`, case-insensitive). LINESTRING / MULTILINESTRING render as colored polylines, POLYGON / MULTIPOLYGON (incl. holes) as stroked alpha-filled shapes, POINT / MULTIPOINT / x,y rows as colored circles. **Multi-geometry picker** — when the result has more than one geometry column (e.g., SHORTEST_PATH's `SOURCE`+`TARGET`+`PATH`, or `match_batch_solves`), a small toolbar above the map lets you pick which column to render, or hit `All` to overlay them on the same canvas. The geometry column is found by datatype first (`tableData.datatypes[i]` containing `wkt` / `geometry` / `geom` — covers `SOLVERS_NODE_WKT (geometry)` and similar typed columns regardless of name), then by header substring (`wkt` / `geom` / `polygon` / `linestring`), then by sniffing the first row's value. All three layers coexist on one map. Auto-fits the viewport to the union bbox. Drag-to-pan, scroll-to-zoom. **Click any feature** (path, polygon, or point — built-in deck.gl pick tolerance acts as the client-side `ST_DWITHIN`) to pop up a detail strip below the map showing the full source row. Available alongside the force-graph tab for the same query when both `nameroute` and `wktroute` columns exist.
...
- **Floating query panels** — Multiple independent panels with minimize (`–` → Q1/Q2 pill in header), maximize, restore, and close. State preserved on minimize/restore — including the **raw Run output** (View Results / Visualization / Map / Video tabs all reappear without re-running the SQL). The cached response also rides into the session save when its JSON is ≤ 2 MB; over that cap the panel restores empty and you re-run manually (Create/Solve/Match wait for your click as usual, Query/Table auto-rerun per the auto-run-queries toggle). **Title-bar drag is clamped to the viewport** — at least 32 px of bar stays visible vertically and 60 px horizontally, so the panel can never be dragged off-screen and lost. Restored positions are also snapped back into reach on mount.
