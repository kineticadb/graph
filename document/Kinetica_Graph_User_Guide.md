...
- **Map visualization** (Deck.gl `SolveMapView`) — a **Map** tab appears whenever the rows include a WKT column or a plain x,y coordinate pair. The geometry column is detected by datatype first (`tableData.datatypes[i]` containing `wkt` / `geometry` / `geom` — handles `SOLVERS_NODE_WKT (geometry)` and similar typed columns regardless of header name), then by header substring (`wkt` / `geom` / `polygon` / `linestring`), then by sniffing the first row's value. Coordinate-pair detection looks for case-insensitive `x`+`y`, `lon`+`lat`, or `longitude`+`latitude`. LINESTRING / MULTILINESTRING render as colored polylines, POLYGON / MULTIPOLYGON (incl. holes) as stroked alpha-filled shapes, POINT / MULTIPOINT / x,y rows as colored circles. All three layers coexist on one map. Auto-fits the viewport to the union bbox. Click any feature for the full source row in a detail strip below the map.
