...
- **Optional `Solution table`** for the result, with an **auto-drop** checkbox (default on) that prefixes the generated SQL with `DROP TABLE IF EXISTS <solution_table>;` so consecutive Runs don't trip on the table left behind by the previous call.
...
- **Constants mode** (`SOURCE_NODES` / `DESTINATION_NODES`) — each section has a `Table | Constants` toggle in its header. Constants mode hides the default-table input and treats each row's value as a literal SQL expression; placeholders adapt per alias (WKT aliases hint `ST_GEOMFROMTEXT('POINT(-122 37)')`, `_ID` aliases hint integers, `_NAME` hints quoted strings, `_TIME` hints timestamps). N-tuple configurations (e.g. `NODE_ID + NODE_WKTPOINT`) collapse into a single combined sub-select: `(SELECT 1 AS ID, ST_GEOMFROMTEXT('POINT(-122 37)') AS WKTPOINT)`.
- **OPTIONS** — key datalist filtered to options applicable to the selected solver (e.g. `convergence_limit` only appears under `PAGE_RANK`).
...
- **Map visualization** (Deck.gl `SolveMapView`) — a **Map** tab appears whenever the rows include either a WKT column (`wktroute`, `wkt`, `polygon`, `geom`, or any column starting with `LINESTRING(` / `POLYGON(` / `MULTILINESTRING(` / `POINT(` / `MULTIPOLYGON(`) or a plain x,y coordinate pair (`x`+`y`, `lon`+`lat`, `longitude`+`latitude`, case-insensitive). All three layer types coexist on one map: `PathLayer` (polylines), `PolygonLayer` (alpha-filled shapes with holes), and `ScatterplotLayer` (circles). Auto-fits the viewport to the union bbox. Click any feature for the full source row in a detail strip below the map.
