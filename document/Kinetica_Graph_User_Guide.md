...
- **Resizable split panes**, **floating helper panels** (minimize → kind-aware color-coded pill — `C*` create / green, `Q*` query / purple, `T*` table preview / purple, `S*` solve / pink, `M*` match / orange — maximize, restore, close), color-coded **progress bar** during data fetch, click-anywhere **node detail strip**, and tooltips on every control.
