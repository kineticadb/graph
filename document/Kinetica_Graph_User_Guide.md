...
<p align="center">
  <img src="images/explorer/explorer_head.webp" alt="Kinetica Graph Explorer" width="900"/>
...
<p align="center">
  <img src="images/explorer/wiki_ontology_flat.webp" alt="Wiki graph ontology — flat labels" width="480"/>
...
<p align="center">
  <img src="images/explorer/wiki_ontology_labelkeys.webp" alt="Wiki graph ontology — grouped by label keys" width="480"/>
...
<p align="center">
  <img src="images/explorer/banking_ontology_fullsearch.webp" alt="Banking graph ontology — full search" width="800"/>
...
<p align="center">
  <img src="images/explorer/banking_session.webp" alt="Banking session with ontology and label charts" width="800"/>
...
<p align="center">
  <img src="images/emergency/seattle_road_graph.webp" alt="osm_seattle road graph — every drivable edge in the Seattle metro region rendered from the OSM-derived edge table" width="700"/>
</p>
...
<p align="center">
  <img src="images/emergency/fsq_fire_stations.webp" alt="FSQ public-places dataset filtered to Fire Stations within the Seattle bounding box — ~1700 candidate origins" width="700"/>
</p>
...
<p align="center">
  <video src="images/emergency/emergency_response.mp4" width="800" autoplay loop muted playsinline
         aria-label="Real-time SVG simulation: every fire station's shortest path to the disaster animates simultaneously over the basemap, the fastest converging first">
...
<p align="center">
  <img src="images/isochrone/grocery_access_table.webp" alt="Result table — per-origin grocery counts within 10-minute isochrones, ranging from 27 (rural source 5) to 600 (urban source 1)" width="700"/>
</p>
...
<p align="center">
  <img src="images/isochrone/grocery_access_map.webp" alt="Map view of the 10 isochrone polygons over the Seattle region, each colored by origin and sized by road-network reachability within 600s" width="700"/>
</p>
...
<p align="center">
  <video src="images/msdo/msdo_dc.mp4" width="500" autoplay loop muted playsinline
         aria-label="Animated SVG of the DC two-depot MSDO solution — trucks fanning out from each depot to deliver customer demands in real time">
    Your browser does not support the video tag.
  </video>
</p>
...
6. From the dashboard header, use **+ Create** (green), **+ Query** (purple), **+ Solve** (pink), or **+ Match** (orange) to open grammar-driven helpers for the corresponding endpoint.
...
<p align="center">
  <img src="images/explorer/explorer_helpers_panels.webp" alt="Graph Explorer — Create / Query / Solve / Match helpers open over ki_home.dc" width="900"/>
  <br/><em>The four grammar-driven helpers in action against <code>ki_home.dc</code>: Create (green, scaffolds a <code>CREATE OR REPLACE DIRECTED GRAPH</code>), Solve (pink, <code>SHORTEST_PATH</code> rendered as colored polylines on Deck.gl), and two Match panels (orange) — <code>match_isochrone</code> rendered first as a polygon and then as a scatter of 7,296 boundary points. Each helper auto-fetches its <code>SOLUTION_TABLE</code> and switches to a Map tab; the DC road network underneath stays interactive throughout.</em>
</p>
...
- Authoring buttons (`+ Create`, `+ Query`, `+ Solve`, `+ Match`) **live in the dashboard header**, not the sidebar — see [Dashboard Helpers](#dashboard-helpers--create--query--solve--match) below.

#### Dashboard Helpers — Create / Query / Solve / Match
The dashboard header carries four color-coded action buttons that open grammar-driven floating panels. All four share the same chrome — draggable, resizable, minimizable to a kind-tagged pill (`C*` green, `Q*` purple, `T*` purple, `S*` pink, `M*` orange) — and all four route their generated SQL through the same `Run` flow with multi-statement support and an auto-fetch of the result table.

##### + Create (green) — author or modify a graph
The Create Helper scaffolds a valid `CREATE OR REPLACE [DIRECTED|UNDIRECTED] GRAPH …` (or `ALTER GRAPH … MODIFY` in Modify mode) without writing the SQL by hand.
...
- **Collapsible NODES / WEIGHTS / RESTRICTIONS** — only `EDGES` is mandatory and stays open; the other three auto-collapse to a single-line pull-down header when empty so the panel doesn't grow tall for nothing.
- **Combo grouping** — every configuration pick tags its row block with a single label header and a `🗑 combo` button that drops the whole group at once. Per-row trash icons still work for fine edits.
- **Default table per section** — type a `schema.table` at the top of any section and the rows accept bare column names; the generator joins them into a single `(SELECT col AS ALIAS, … FROM <default>)` sub-select. Rows that contain a dot are treated as full overrides and split into their own sub-selects (split-table sections still work).
- **OPTIONS** — `+ Add option` lets you add arbitrary `key = 'value'` pairs that get emitted as `OPTIONS => KV_PAIRS(...)`. A shared datalist suggests common option keys.
...
##### + Query (purple) — GQL with the Query Helper
See [SQL / GQL Query](#sql--gql-query) below for the full description. The same panel kind is also reused for **table previews** (`T*` pill, opened from the sidebar's right-click → Open Preview rows / Schema), in which case the Query Helper is hidden.

<p align="center">
  <img src="images/explorer/explorer_solve_match_video.webp" alt="Graph Explorer — Solve and Match helpers with Video, polyline, polygon, and point map visualizations over ki_home.san_francisco" width="900"/>
  <br/><em>Solve / Match helpers in action on <code>ki_home.san_francisco</code> (1.24 M nodes / 1.34 M edges). Left → right: a <code>markov_chain</code> Match panel with the <strong>Video</strong> tab playing an inline animated SVG of GPS map-matching from <code>info.svg</code>; a <code>SHORTEST_PATH</code> Solve rendered as colored polylines (LINESTRING); a <code>match_isochrone</code> Match showing the POLYGON boundary (output index 1); and the same isochrone with <code>result_table_index='2'</code> picked from the Output dropdown — auto-fetched from <code>&lt;solution_table&gt;_polygons</code> and rendered as a ScatterplotLayer of 2,784 boundary points. The auto-prefilled <code>SOLUTION_TABLE</code> values (<code>ki_home.solution_san_francisco_shortest_path</code>, etc.) and pre-loaded grammar-default OPTIONS (<code>inverse_solve='false'</code>, <code>max_radius='120'</code>) are visible in each panel.</em>
</p>

##### + Solve (pink) — `SOLVE_GRAPH` builder
A grammar-driven helper for the `/solve/graph` endpoint that mirrors the Create Helper's UX for non-NODES/EDGES components.

- **Solver type** dropdown (`SHORTEST_PATH`, `PAGE_RANK`, `STATS_ALL`, …) drives configuration filtering throughout the panel — only configurations and OPTIONS whose `filtervalues` include the selected solver are offered.
- **`Solution table`** is auto-prefilled with `<schema>.solution_<graph>_<solver>` (e.g. `ki_home.solution_san_francisco_shortest_path`) and refreshes whenever you change solver type, unless you've customized the name. Two SQL forms are keyed on this field:
  - **Field set** → `EXECUTE FUNCTION SOLVE_GRAPH(... SOLUTION_TABLE => '<t>' ...)` — persists results to the named table, with the **auto-drop** checkbox (default on) prefixing `DROP TABLE IF EXISTS <solution_table>;` so consecutive Runs don't trip on a table left behind.
  - **Field cleared** → `SELECT * FROM SOLVE_GRAPH(...)` (and `SELECT * FROM MATCH_GRAPH(...)` for Match) — returns rows inline without materializing a persistent table.
- **Output dropdown** next to the Solution table input — when the current solver/method has multiple output schemas (e.g. `match_isochrone` has two: an x/y/z node table and a POLYGON table), a `Output: [default | 1: … | 2: …]` picker appears with each schema's columns summarized (and `(wkt)` flagged on geometry columns). Picking a non-default value injects `result_table_index = 'N'` into the generated OPTIONS automatically — no need to add it by hand.
- **Component sections** — `WEIGHTS`, `RESTRICTIONS`, `SOURCE_NODES`, `DESTINATION_NODES`. Same UX as the Create Helper (configuration dropdowns, default-table, combo grouping, column auto-complete, collapsible headers). For `SOURCE_NODES` / `DESTINATION_NODES` the row alias collapses to the generic `NODE` — Kinetica infers ID / NAME / WKTPOINT from the column data.
- **Constants mode** (`SOURCE_NODES` / `DESTINATION_NODES`) — each section has a `Table | Constants` toggle in its header. Constants mode hides the default-table input and treats each row's value as a literal SQL expression. **Focus-fill templates** — clicking into an empty constants field auto-drops the alias-appropriate template (`ST_GEOMFROMTEXT('POINT(-122 37)')` for WKT, `'Bill'` for NAME, `1` for ID, timestamp examples for TIME) and pre-selects the editable portion (the coordinates / quoted value) so a single keystroke replaces it. N-tuple configurations (e.g. `NODE_ID + NODE_WKTPOINT`) collapse into a single combined sub-select: `(SELECT 1 AS ID, ST_GEOMFROMTEXT('POINT(-122 37)') AS WKTPOINT)`.
- **OPTIONS** auto-populates with grammar defaults for the selected solver — every option whose `filtervalues` mentions the current solver is pre-filled with its default (e.g. `match_isochrone` pre-loads `max_radius = 0.0`). Switching solver replaces the auto-supplied rows with the new solver's defaults; rows you've added or edited yourself survive. **Boolean options** (per grammar `type: 'boolean'`) render as a `true`/`false` dropdown — common toggles like `inverse_solve` / `output_clusters` / `force_undirected` flip with a click. The key datalist is filtered to options applicable to the current solver.
- **Sticky Generate button** — the Generate row stays pinned at the bottom of the helper's scroll wrapper, so it's always reachable no matter how many rows you've added.
- **Run flow** — multi-statement execution splits on `;` (stripping `--` and `/* */` comments) and runs each statement sequentially. After a successful solve with `SOLUTION_TABLE`, the panel auto-runs `SELECT * FROM <solution_table> LIMIT 10000` and lands the rows in the **View Results** tab — same flow as a GQL query's hop results.
- **Path visualization** (force-graph) — when the auto-fetched rows include a `nameroute` column (emitted by `SHORTEST_PATH`, `INVERSE_SHORTEST_PATH`, `ALLPATHS`, `MULTIPLE_ROUTING`, `BACKHAUL_ROUTING`) a Visualization tab appears with each row as one colored path.
- **Map visualization** (Deck.gl `SolveMapView`) — a **Map** tab appears whenever the rows include a WKT column or a plain x,y coordinate pair. Geometry-column detection runs all three steps and unions the results: datatype (`tableData.datatypes[i]` containing `wkt` / `geometry` / `geom`), header substring (`wkt` / `geom` / `polygon` / `linestring`), and value sniffing (first row starts with `LINESTRING(` / `POINT(` / etc.). Coordinate-pair detection looks for case-insensitive `x`+`y`, `lon`+`lat`, or `longitude`+`latitude`. LINESTRING / MULTILINESTRING render as colored polylines, POLYGON / MULTIPOLYGON (incl. holes) as stroked alpha-filled shapes, POINT / MULTIPOINT / x,y rows as colored circles. All three layers coexist on one map. **Multi-geometry picker** — when the result row exposes more than one geometry column (e.g. SHORTEST_PATH's `SOURCE` + `TARGET` + `PATH`, or `match_batch_solves`), a small toolbar above the map lets you pick which column to render or hit **`All`** to overlay every detected column on the same canvas. Auto-fits the viewport to the union bbox. Click any feature for the full source row in a detail strip below the map.
- **Video visualization** — when the solver/matcher response carries an inline animated SVG in `info.svg`, a **Video** tab appears alongside Map with the SVG rendered directly. Its own SMIL / CSS animations play in place — common for supply-demand routes, GPS map-matching trails, and similar timeline outputs.
- The helper auto-collapses (`▶ Solve Helper`) once a viz tab is shown so the result claims the full top region. Click the header to re-expand and tweak.

##### + Match (orange) — `MATCH_GRAPH` builder
Mirror of the Solve Helper for the `/match/graph` endpoint.

- **Solve method** dropdown (`markov_chain` for GPS map-matching, `match_supply_demand`, `match_od_pairs`, `match_loops`, `match_charging_stations`, `match_route_detour`, `match_isochrone`, …) drives configuration, option, and add-on filtering.
- Single component **`SAMPLE_POINTS`** — same UX as Solve's `SOURCE_NODES` (configuration dropdown filtered by method, default-table, combo grouping, `Table | Constants` toggle). The `SAMPLE_` prefix is stripped from row aliases (e.g. `SAMPLE_WKTPOINT` → `WKTPOINT`, `SAMPLE_ORIGIN_WKTPOINT` → `ORIGIN_WKTPOINT`).
- **Add-on filtering by domain** — picking a configuration only spawns optional rows whose prefix matches the method's domain (`SAMPLE_SUPPLY_*` only for `match_supply_demand`, `SAMPLE_PICKUP_*` for `match_pickup_dropoff`, `SAMPLE_OD_*` for `match_od_pairs`, etc.) — no 20-row noise on simple methods.
- Same auto-drop, multi-statement Run, auto-fetch solution rows, and Path / Map viz tabs as Solve. For `match_isochrone` with `result_table_index = '2'`, the auto-fetch targets the `<solution_table>_polygons` sibling (with an `INFORMATION_SCHEMA.TABLES LIKE '<solution_table>_%'` probe fallback when the suffix mapping misses).
...
- **Click anywhere on the Deck.gl map** (edge or empty area) copies just `<lon> <lat>` (space-separated, no `ST_GEOMFROMTEXT` wrapper) to the clipboard with a `✓ copied` overlay. Pairs with the Solve / Match Constants-mode focus-fill template — opening an empty WKT field drops in `ST_GEOMFROMTEXT('POINT(x y)')` with `x y` pre-selected, so a single Ctrl+V replaces exactly those coords. Edge clicks still surface the source-table row in the detail strip.
...
<p align="center">
  <img src="images/explorer/explorer_geo_rendering.webp" alt="Graph Explorer geo rendering modes" width="900"/>
...
| **WMS** *(server-side)* | Kinetica `/wms` endpoint renders PNG tiles on the database side | Tens of millions of edges; client RAM stays at zero — only PNG bytes cross the wire | Style sub-toggle `Auto / Heat / Raster / CB` — Heatmap uses the jet colormap with a zoom-aware blur radius so the spectrum stays consistent at every zoom; CB shows class-break coloring by edge label (top 20). Cursor-centered wheel zoom, debounced pan. **Auth-aware** — tiles are fetched with `fetch()` + Basic-auth header → blob URL → `<img>`, so deployments behind authentication don't 401 on plain `<img src>` requests |
...
- **Cached Run output** — each panel's last `/execute/sql` response is included in the session when its JSON is **≤ 2 MB**. On load, those panels reopen with View Results / Visualization / Map / Video tabs already populated and `autoRun` forced off (no re-execution). Over-cap panels save empty; on restore, **Query** and **Table** panels auto-rerun if the Auto-run-queries toggle is on, while **Create / Solve / Match** wait for an explicit Run click so big match outputs aren't silently re-fired.
...
- **Resizable split panes**, **floating helper panels** (minimize → kind-aware color-coded pill — `C*` create / green, `Q*` query / purple, `T*` table preview / purple, `S*` solve / pink, `M*` match / orange — maximize, restore, close). Minimize → restore brings the **raw Run output** (View Results / Visualization / Map / Video tabs) back verbatim without re-running the SQL. Color-coded **progress bar** during data fetch, click-anywhere **node detail strip**, and tooltips on every control.
- **Per-kind sequential numbering** — `C1/C2…`, `Q1/Q2…`, etc., is stable across the full panel list (not just minimized ones), so a pill keeps the same label across minimize / restore cycles.
- **Title-bar drag is viewport-clamped** — at least 32 px of the title bar stays visible vertically and 60 px horizontally, so a panel can never be dragged off-screen and lost. Restored positions are snapped back into reach on mount too.
...
| deck.gl v9 | WebGL GPU-accelerated geo visualization (`LineLayer` for graph edges; `PathLayer` + `PolygonLayer` + `ScatterplotLayer` in `SolveMapView` for Solve / Match outputs) |
...
<p align="center">
  <img src="images/explorer/top_movies_session.webp" alt="top_movies session — ontology, label charts, and visualization" width="900"/>
...
<p align="center">
  <img src="images/explorer/top_movies_q0_results.webp" alt="Top movies — actor ranking table" width="900"/>
...
<p align="center">
  <img src="images/explorer/top_movies_q0_graph.webp" alt="Top movies — actor/movie co-appearance graph" width="900"/>
...
<p align="center">
  <img src="images/explorer/banking_session.webp" alt="Banking session — ontology, label charts, and visualization" width="900"/>
...
<p align="center">
  <img src="images/explorer/banking_q0_results.webp" alt="Banking — all-bank trail results" width="900"/>
...
<p align="center">
  <img src="images/explorer/banking_q0_graph.webp" alt="Banking — all-bank trail" width="900"/>
...
<p align="center">
  <img src="images/explorer/banking_q1_results.webp" alt="Banking — single-bank drilldown results" width="900"/>
  <br/><em>Explorer View Results tab — 55 trail rows, all originating from the single pinned bank UUID</em>
</p>

<p align="center">
  <img src="images/explorer/banking_q1_graph.webp" alt="Banking — single-bank drilldown" width="900"/>
...
<p align="center">
  <img src="images/explorer/wiki_q0_graph.webp" alt="Wiki — chess chain traversal" width="900"/>
...
<p align="center">
  <img src="images/explorer/wiki_q1_results.webp" alt="Wiki — variable-length friend path results" width="900"/>
...
<p align="center">
  <img src="images/explorer/wiki_q1_graph.webp" alt="Wiki — variable-length friend paths" width="900"/>
